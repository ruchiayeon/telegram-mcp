...
_default_user = _config.get("default_user", "gayeon")
_user_cfg = _config.get(_default_user, {})
...
class Config:
    # Telegram 인증
    API_ID: int = int(_user_cfg.get("api_id", "0") or "0")
    API_HASH: str = _user_cfg.get("api_hash", "")
    PHONE: str = _user_cfg.get("phone", "")

    # 세션 파일명 (telegram_session.session 파일로 저장됨)
    SESSION_NAME: str = _settings.get("session_name", "telegram_session")
